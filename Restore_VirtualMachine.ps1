...
#         .Synopsis                                                                     #
#         .Description                                                                  #
#       The script will restore encrypted virtual machine with (DEK Only) from          #
...
Switch ($qresult) {
    0 {
	    ...
	    try 
		{
		    Set-AzureRmVMOSDisk -VM $vm -ManagedDiskId $osDisk.Id -DiskEncryptionKeyUrl $dekUrl -DiskEncryptionKeyVaultId $dekVaultId -CreateOption "Attach" -Windows
    ...
    1 {
	    ...
	    try 
		{
		    Set-AzureRmVMOSDisk -VM $vm -Name $osDiskName -VhdUri $osVhdUri -DiskEncryptionKeyUrl $dekUrl -DiskEncryptionKeyVaultId $dekVaultId -Caching $caching -CreateOption "Attach" -Windows/Linux -StorageAccountType $storageType
